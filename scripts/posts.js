...
Api.getPosts().then(function (posts) {
    posts
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    wrapper = document.getElementById("posts-wrapper-home");
                    return [4 /*yield*/, post.makeSmallContainer()];
                case 1:
                    postObj = _a.sent();
                    postObj.style.height = "100%";
                    title = postObj.querySelector(".post-title");
                    body = postObj.querySelector(".post-body");
                    title.addEventListener("click", function (event) {
                        var posts = Array.from(document.getElementsByClassName("post-obj"));
                        var prevDisplay = body.style.display;
                        posts.forEach(function (p) {
                            var bodyElem = p.querySelector(".post-body");
